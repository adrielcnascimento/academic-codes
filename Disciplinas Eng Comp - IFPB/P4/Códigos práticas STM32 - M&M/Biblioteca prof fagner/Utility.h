...
void Configure_Clock(void)
{
	...
	RCC->CIR = 0x00000000;		//desabilita todas as interrupções
	RCC->CR |= 0x00000001;		//liga o oscilador HSI
	RCC->CFGR = 0x00000000;		//reseta o registrador CFGR
	RCC->CR &= 0xFEF6FFFF;		//desliga HSE, CSS e o PLL
	RCC->PLLCFGR = 0x24003010;	//reseta o registrador PLLCFGR
	RCC->CR &= 0xFFFBFFFF;		//reseta o bypass de HSE

	//Configura a fonte de clock (HSE), os fatores de divisão e multiplicação do PLL,
	//prescalers dos barramentos AHB, APB, estados de espera da flash e
	//habilitação das memórias cache de instruções e dados
	RCC->CR |= 0x00010000;				//habilita HSE
	while(!((RCC->CR) & 0x00020000));	//espera HSE ficar pronto verificando a flag HSERDY

    RCC->APB1ENR |= 0x10000000;			//habilita o clock da interface do regulador de tensão
    PWR->CR |= 0x0000C000;				//Seleciona modo 1 da saída do regulador de tensão
    ...
    while(!(RCC->CR & 0x02000000));		//espera o PLL ficar pronto verificando a flag PLLRDY

    //habilita a pré-busca na flash, configura os estados de espera (= 5 estados),
    //habilita a cache de instruções e de dados
...
void USART1_Init(void)
{
	...
	GPIOA->MODER |= (0b10 << 20) | (0b10 << 18) ;	//pinos PA9 e PA10 como função alternativa
	GPIOA->AFR[1] |= (0b0111 << 8) | (0b0111 << 4);	//função alternativa 7 (USART1)
}


//redefinição da função de envio de caracteres pela USART1 utilizada no redirecionamento de printf
...
//redefinição da função de recebimento de caracteres pela USART1 utilizada no redirecionamento de scanf
